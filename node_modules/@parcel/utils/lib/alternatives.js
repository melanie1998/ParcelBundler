...
async function findAlternativeNodeModules(fs, moduleName, dir) {
  ...
  while (dir !== root) {
    ...
    try {
      ...
      if (stats.isDirectory()) {
        let dirContent = await fs.readdir(modulesDir); // Filter out the modules that interest us
        ...
        if (isOrganisationModule) {
          await Promise.all(modules.map(async item => {
            ...
            let orgDirContent = await fs.readdir(orgDirPath); // Add all org packages
  ...
  return (0, _schema().fuzzySearch)(potentialModules, moduleName).slice(0, 2);
}
