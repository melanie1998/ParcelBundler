...
const VISITOR = {
  ...
  AssignmentExpression(path, asset) {
    ...
    if ((0, t().isMemberExpression)(left) && ((0, t().isIdentifier)(left.object, {
      ...
      if (!scope.hasBinding(identifier.name)) {
        asset.symbols.set(name, identifier.name, (0, _babelAstUtils().convertBabelLoc)(path.node.loc)); // If in the program scope, create a variable declaration and initialize with the exported value.
  ...
  CallExpression(path, asset) {
    ...
    if (isRequire) {
      ...
      dep.symbols.set('*', (0, _utils().getName)(asset, 'require', source), (0, _babelAstUtils().convertBabelLoc)(path.node.loc)); // Generate a variable name based on the current asset id and the module name to require.
      ...
      for (let specifier of (0, _nullthrows().default)(specifiers)) {
        ...
        if (dep && imported) {
          ...
          if (existing) {
            ...
            let loc = (0, _babelAstUtils().convertBabelLoc)(specifier.loc);
            asset.addDependency({
  ...
  ExportAllDeclaration(path, asset) {
    ...
    if (dep) {
      dep.symbols.set('*', '*', (0, _babelAstUtils().convertBabelLoc)(path.node.loc));
    }
