...
function shake(ts, moduleGraph, context, sourceFile) {
  ...
  let visit = node => {
      ...
      if (!node.moduleSpecifier || moduleGraph.getModule(node.moduleSpecifier.text)) {
        if (!node.moduleSpecifier && node.exportClause) {
          ...
          if (exported.length > 0) {
            return ts.updateExportDeclaration(node, undefined, // decorators
            undefined, // modifiers
            ts.updateNamedExports(node.exportClause, exported), undefined // moduleSpecifier
            );
          }
        }
    ...
    if ((0, _utils().isDeclaration)(ts, node)) {
      let name = (0, _utils().getExportedName)(ts, node) || node.name.text; // Remove unused declarations
      ...
      node.modifiers = (node.modifiers || []).filter(m => m.kind !== ts.SyntaxKind.ExportKeyword && m.kind !== ts.SyntaxKind.DefaultKeyword); // Rename declarations
      ...
      if (exportedNames.get(newName) === currentModule) {
        if (newName === 'default') {
          node.modifiers.unshift(ts.createModifier(ts.SyntaxKind.DefaultKeyword));
        }
    ...
    if (ts.isVariableStatement(node)) {
      node = ts.visitEachChild(node, visit, context); // Remove empty variable statements
      ...
      node.modifiers = (node.modifiers || []).filter(m => m.kind !== ts.SyntaxKind.ExportKeyword && m.kind !== ts.SyntaxKind.DeclareKeyword); // Add export modifier if all declarations are exported.
    ...
    if (ts.isQualifiedName(node) && ts.isIdentifier(node.left)) {
      ...
      if (resolved && resolved.module.hasBinding(resolved.name)) {
        ...
        return ts.updateQualifiedName(node, ts.createIdentifier(currentModule.getName(node.left.text)), node.right);
      }
    } // Remove private properties


    if (ts.isPropertyDeclaration(node)) {
      let isPrivate = node.modifiers && node.modifiers.some(m => m.kind === ts.SyntaxKind.PrivateKeyword);
...
function generateImports(ts, moduleGraph) {
  ...
  for (let [specifier, names] of moduleGraph.getAllImports()) {
    ...
    if (namespaceSpecifier) {
      let importClause = ts.createImportClause(defaultSpecifier, namespaceSpecifier);
      importStatements.push(ts.createImportDeclaration(undefined, undefined, importClause, // $FlowFixMe
      ts.createLiteral(specifier)));
      defaultSpecifier = undefined;
    }

    if (defaultSpecifier || namedSpecifiers.length > 0) {
      let importClause = ts.createImportClause(defaultSpecifier, namedSpecifiers.length > 0 ? ts.createNamedImports(namedSpecifiers) : undefined);
      importStatements.push(ts.createImportDeclaration(undefined, undefined, importClause, // $FlowFixMe
      ts.createLiteral(specifier)));
    }
  }
