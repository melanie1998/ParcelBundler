...
var _default = new (_plugin().Packager)({
  async package({
    ...
    let referenced = new Set(bundleGraph.getReferencedBundles(bundle).map(b => b.id));
    let bundles = bundleGraph.getSiblingBundles(bundle).filter(b => !b.isInline && !referenced.has(b.id));
    ...
    let {
      html
    } = await (0, _posthtml().default)([insertBundleReferences.bind(this, bundles), replaceInlineAssetContent.bind(this, bundleGraph, getInlineBundleContents)]).process(code, renderConfig);
    let {
      contents,
      map
